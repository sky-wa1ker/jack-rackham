...
async def nation(ctx, *, nation_name):
    if ctx.channel.name == ('tech_support'):
        ...
        if result_dict is False:
            ...
            embed.add_field(name="VM-Beige", value=f'`In VM: For {v_mode_turns} turns. | In Beige: For {beige_turns} turns.`', inline=False)
            embed.add_field(name="War Range", value=f'`⬆️ {round((score * 0.75),2)} to {round((score * 1.75),2)} | ⬇️ {round((score / 1.75),2)} to {round((score / 0.75),2)}`', inline=False)
            embed.add_field(name="Wars", value=f'`⬆️ {o_wars} | ⬇️ {d_wars}`', inline=False)
            embed.add_field(name="Military", value=f'`💂 {soldiers} | ⚙️ {tanks} | ✈️ {aircraft} | 🚢 {ships} | 🚀 {missiles} | ☢️ {nukes}`', inline=False)
            embed.set_footer(text="DM Sam Cooper for help.")
            await ctx.send(embed=embed)
    else:

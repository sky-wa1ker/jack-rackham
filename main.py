...
async def counter(ctx, *, enemy_id):
    if ctx.channel.category.name != 'PUBLIC':
        if enemy_id is int:
            await ctx.send('It is all pointless.')
        else:
            async with aiohttp.ClientSession as session:
                async with session.get(f'https://politicsandwar.com/api/nation/id={enemy_id}&key={api_key}') as r:
                    ...
                    if enemy_data['success'] is False:
                        ...
                        async with aiohttp.ClientSession as session2:
                            async with session2.get(f'https://politicsandwar.com/api/v2/nations/{api_key}/&alliance_id=913&v_mode=0&min_score={min_score}&max_score={max_score}') as r:
                                test_request = await r.json()
